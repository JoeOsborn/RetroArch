...
bool bsv_movie_load_checkpoint(bsv_movie_t *handle, uint8_t compression, uint8_t encoding, replay_checkpoint_behavior checkpoint_behavior)
{
   ...
   switch (encoding)
   {
      ...
      case REPLAY_CHECKPOINT2_ENCODING_STATESTREAM:
         if(!bsv_movie_read_deduped_state(handle, encoded_data, encoded_size, (checkpoint_behavior == REPLAY_CPBEHAVIOR_DESERIALIZE)))
         {
...
{
   /*
     TODO: It would be good to allow this for recordings when paused
     and not at the end of the recording.
    */
